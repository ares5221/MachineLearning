...
def load_data_set():
    ...
    data_arr = []
    label_arr = []
    # localdir = 'D:\python\MachineLearning\db\5.Logistic'
    f = open('D:/python/MachineLearning/db/5.Logistic/TestSet.txt', 'r')
...
def plot_best_fit(weights):
    ...
    ax.scatter(x_cord1, y_cord1, s=30, color='green', marker='^')
...
def test():
    """
    这个函数只要就是对上面的几个算法的测试，这样就不用每次都在power shell 里面操作
    :return: 
    """
...
def colic_test():
    ...
    :return: 
    """
    f_train = open('D:/python/MachineLearning/db/5.Logistic/HorseColicTraining.txt', 'r')
    f_test = open('D:/python/MachineLearning/db//5.Logistic/HorseColicTest.txt', 'r')
    training_set = []
    training_labels = []
